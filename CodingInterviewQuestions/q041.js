...
const find = (arr, current = []) => {
  if (arr.length === 0) {
    return current;
  }
  const [value, ...rest] = arr;
  const arr1 = find(rest, current);
  const arr2 = current.length === 0 || value > current[current.length - 1] ? find(rest, [...current, value]) : [];

  return arr1.length > arr2.length ? arr1 : arr2;
}

console.log(find([0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15]));